...
<title>RONIN_COLLECT | Login</title>
...
<body class="bg-background text-on-background font-body min-h-screen flex items-center justify-center p-6 lg:p-12">
    <div class="w-full max-w-[1200px] flex flex-col lg:flex-row gap-12 bg-white shadow-2xl p-8 lg:p-16 relative overflow-hidden">
        ...
        <div class="lg:w-1/2 flex flex-col justify-between border-b lg:border-b-0 lg:border-r border-outline/20 pb-12 lg:pb-0 lg:pr-12 relative z-10">
            <div>
                <span class="font-label text-primary tracking-[0.2em] text-[10px] font-bold uppercase mb-4 block">Authorization Module</span>
                ...
                <p class="font-body text-xl opacity-70 max-w-sm">Secure terminal for curators and archivists.</p>
            </div>
            <div class="mt-12 lg:mt-0 font-label text-[10px] tracking-widest uppercase opacity-40">
                System Version 1.0.4 // Kyoto Branch
            </div>
        </div>
        
        <!-- Login Form -->
        <div class="lg:w-1/2 flex flex-col justify-center relative z-10">
            <h2 class="font-headline font-extrabold text-3xl uppercase tracking-tighter mb-8">Access Required</h2>
            
            <form action="{{ route('login') }}" method="POST" class="space-y-6">
                @csrf
                
                <div>
                    <label class="font-label text-[10px] uppercase tracking-widest opacity-70 block mb-2">Curator ID / Username</label>
                    <input name="name" value="{{ old('name') }}" type="text" required autofocus class="w-full bg-surface-container-low border-0 border-b-2 border-transparent focus:border-primary px-4 py-4 font-body placeholder:opacity-40 transition-colors" placeholder="e.g. curator_01"/>
                    @error('name')
                ...
                <div>
                    <label class="font-label text-[10px] uppercase tracking-widest opacity-70 block mb-2">Passcode</label>
                ...
                <div class="flex items-center gap-3 pt-2 pb-4">
                    ...
                    <label for="remember" class="font-label text-[10px] uppercase tracking-widest opacity-70">Remember Terminal</label>
                </div>
