...
<form action="{{ route('comics.update', $comic) }}" method="POST" class="grid grid-cols-1 xl:grid-cols-12 gap-12" enctype="multipart/form-data">
    ...
    <div class="xl:col-span-8 space-y-10">
        ...
        <input name="author" value="{{ old('author', $comic->author) }}" required class="w-full bg-surface-container-low border-0 focus:ring-2 focus:ring-primary px-4 py-3 font-body text-on-surface placeholder:text-outline/40" placeholder="misal: Akira Toriyama" type="text"/>
        ...
        <select name="status" required class="w-full bg-surface-container-low border-0 focus:ring-2 focus:ring-primary px-4 py-3 font-body text-on-surface uppercase text-sm tracking-widest">
            <option value="plan_to_read" {{ old('status', $comic->status) == 'plan_to_read' ? 'selected' : '' }}>Rencana Dibaca</option>
            <option value="reading" {{ old('status', $comic->status) == 'reading' ? 'selected' : '' }}>Sedang Dibaca</option>
            <option value="completed" {{ old('status', $comic->status) == 'completed' ? 'selected' : '' }}>Selesai</option>
            <option value="wishlist" {{ old('status', $comic->status) == 'wishlist' ? 'selected' : '' }}>Incaran / Wishlist</option>
        </select>
        </div>
        ...
        <select name="priority" class="w-full bg-surface-container-low border-0 focus:ring-2 focus:ring-primary px-4 py-3 font-body text-on-surface uppercase text-sm tracking-widest">
            ...
            <option value="low" {{ old('priority', $comic->priority) == 'low' ? 'selected' : '' }}>Rendah</option>
            <option value="medium" {{ old('priority', $comic->priority) == 'medium' ? 'selected' : '' }}>Menengah</option>
            <option value="high" {{ old('priority', $comic->priority) == 'high' ? 'selected' : '' }}>Tinggi</option>
            <option value="extreme" {{ old('priority', $comic->priority) == 'extreme' ? 'selected' : '' }}>Ekstrem</option>
        </select>
        </div>
        ...
        <input name="price" value="{{ old('price', $comic->price) }}" type="number" step="1000" class="w-full bg-surface-container-low border-0 focus:ring-2 focus:ring-primary px-4 py-3 font-body text-on-surface placeholder:text-outline/40" placeholder="40000"/>
        </div>
        </div>
        ...
        <input name="tags_input" value="{{ old('tags_input', $tagsString ?? '') }}" class="w-full bg-surface-container-low border-0 focus:ring-2 focus:ring-primary px-4 py-3 font-body text-on-surface placeholder:text-outline/40 uppercase text-sm tracking-widest" placeholder="misal: Action, Shonen" type="text"/>
        </div>

        <!-- Description -->
        <div>
        <label class="font-label text-[10px] uppercase tracking-widest text-on-surface-variant block mb-2">Sinopsis Naratif</label>
        <textarea name="description" class="w-full bg-surface-container-low border-0 focus:ring-2 focus:ring-primary px-4 py-4 font-body text-on-surface leading-relaxed resize-none" placeholder="Deskripsikan perjalanan, dunianya, dan keadaannya..." rows="6">{{ old('description', $comic->description) }}</textarea>
        </div>
